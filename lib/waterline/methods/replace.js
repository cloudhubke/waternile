...
var DEFERRED_METHODS = getQueryModifierMethods('replace');

/**
 * replace()
 *
 * Replace records that match the specified criteria, patching them with
 * the provided values.
 *
 * ```
 * // Forgive all debts: Zero out bank accounts with less than $0 in them.
 * BankAccount.replace().where({
 *   balance: { '<': 0 }
 * }).set({
 *   balance: 0
 * }).exec(function(err) {
      ...
      try {
        ...
        switch (e.code) {
              ...
              flaverr(
                {
                  ...
                  details: e.details,
                  message:
                    'Cannot perform replace with the provided values.\n' +
                    'Details:\n' +
                    '  ' +
                    e.details +
                    '\n',
          ...
          case 'E_NOOP':
            ...
            // >  See: https://github.com/treelinehq/waterline-query-docs/blob/master/docs/results.md#replace
      ...
      (function (proceed) {
          ...
          function _eachKnownAttrDef(attrDef, attrName) {
            if (attrDef.collection) {
              ...
              if (query.valuesToSet[attrName]) {
                ...
                // doesn't need to do anything during the initial replace.
        ...
        adapter.replace(
          ...
          function _afterTalkingToAdapter(err, rawAdapterResult) {
            if (err) {
              err = forgeAdapterError(err, omen, 'replace', modelIdentity, orm);
              return done(err);
            } //-•
              ...
              if (!_.isUndefined(rawAdapterResult)) {
                console.warn(
                  '\n' +
                    ...
                    "from its `replace` method.  But it did!  And since it's an array, displaying this\n" +
              ...
              return done(
                new Error(
                  'Unexpected behavior in database adapter: Since `fetch: true` was enabled, this adapter ' +
                    ...
                    '2nd argument when triggering the callback from its `replace` method.  But instead, got: ' +
                    util.inspect(rawAdapterResult, { depth: 5 }) +
                    ''
                )
              );
