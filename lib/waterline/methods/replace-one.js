...
var DEFERRED_METHODS = getQueryModifierMethods('replaceOne');

/**
 * replaceOne()
 *
 * Replace a single record that matches the specified criteria, patching it with
 * the provided values and returning the replaced record.
 *
 * @experimental
 *
...
module.exports = function replaceOne(
  criteria,
  valuesToSet,
  explicitCbMaybe,
  metaContainer
  ...
  var omenMaybe = flaverr.omen(replaceOne);
    ...
    function (done) {
      ...
      WLModel.count(
        ...
        function _afterCounting(err, total) {
          ...
          var modifiedMetaForReplace = _.extend({}, query.meta || {}, {
            fetch: true,
            skipEncryption: true,
          });

          var modifiedCriteriaForReplace = _.omit(query.criteria, [
            'select',
            'omit',
            'limit',
            'skip',
